...
export default function SpyPage() {
  ...
  return (
    <div className="max-w-2xl space-y-8">
      <div>
        <p className="font-mono text-xs uppercase tracking-[0.3em] text-redact">Dossier confidentiel</p>
        <h1 className="font-display text-2xl uppercase tracking-wide text-brass">Réseau d&apos;espionnage</h1>
      </div>
        ...
        {targets.map((t) => (
          <div key={t.id} className="dossier-card flex items-center justify-between p-4">
            ...
            <button
              disabled={!!onCooldown}
              onClick={() => spyOn(t.id)}
              className="border border-redact px-4 py-1 font-mono text-xs uppercase text-redact hover:bg-redact hover:text-paper disabled:opacity-30"
            >
              Espionner
            </button>
          </div>
