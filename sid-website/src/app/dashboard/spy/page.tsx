  ...
  return (
    <div className="max-w-2xl space-y-8">
      ...
      <div className="glass-card space-y-2 p-6">
        ...
        <p className="font-mono text-xs text-paper/70">{spyRole.xp} XP{nextLevel ? ` · ${nextLevel.next - spyRole.xp} XP avant le niveau ${nextLevel.label}` : " · niveau maximum atteint"}</p>
      ...
      <div className="space-y-3">
        ...
        {reports.map((r) => (
          <div key={r.id} className="glass-card space-y-2 p-4">
            <p className="font-mono text-xs text-paper/60">{new Date(r.created_at).toLocaleString("fr-FR")}</p>
            {r.snippets.map((s, i) => (
