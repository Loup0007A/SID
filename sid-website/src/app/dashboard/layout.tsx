...
const NAV: { href: string; label: string; perm?: PermissionKey[] }[] = [
  ...
  { href: "/dashboard/admin/roles", label: "Rôles & équipes", perm: ["manage_roles", "manage_teams"] },
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-ink px-4">
        <div className="dossier-card max-w-md space-y-3 p-8 text-center">
          ...
          <button onClick={handleLogout} className="font-mono text-xs uppercase text-redact underline">
            Se déconnecter
          </button>
        </div>
      </main>
  ...
  return (
    <div className="flex min-h-screen bg-ink text-paper">
      <aside className="grain-panel hidden w-64 shrink-0 flex-col border-r border-ink-border p-5 md:flex">
        <div className="mb-8">
          <p className="font-display text-2xl uppercase tracking-wide text-brass">S.I.D.</p>
          <p className="font-mono text-xs text-paper/50">{profile?.nickname}</p>
          {profile?.is_founder && <span className="stamp mt-1 text-brass">Fondateur</span>}
        </div>
        ...
        <nav className="flex-1 space-y-1">
          {NAV.filter((item) => !item.perm || can(permissions, ...item.perm)).map((item) => (
            <Link
              ...
              href={item.href}
              className={clsx(
                "block rounded px-3 py-2 font-mono text-sm uppercase tracking-wide hover:bg-ink-border",
                pathname === item.href && "bg-ink-border text-brass"
              )}
            >
              {item.label}
            </Link>
          ...
          {isSpy && (
            <Link
              href="/dashboard/spy"
              className={clsx(
                "block rounded px-3 py-2 font-mono text-sm uppercase tracking-wide text-redact hover:bg-ink-border",
        ...
        <button
          ...
          className="mt-4 border border-paper/20 px-3 py-2 font-mono text-xs uppercase text-paper/70 hover:border-redact hover:text-redact"
        >
          Se déconnecter
        </button>
      </aside>
      ...
      <div className="flex-1">
        <div className="border-b border-ink-border p-4 md:hidden">
          <p className="font-display text-xl uppercase text-brass">S.I.D.</p>
        </div>
