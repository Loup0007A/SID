  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl uppercase tracking-wide text-brass">Comptoir de la S.I.D.</h1>
          {wallet && <p className="font-mono text-sm text-olive">Solde : {wallet.balance.toLocaleString("fr-FR")} Cr.</p>}
        </div>
        {canManage && (
          <button onClick={() => setShowForm((s) => !s)} className="bg-brass px-4 py-2 font-display text-sm uppercase text-ink hover:bg-brass-light">
            {showForm ? "Annuler" : "Nouvel objet"}
          </button>
        )}
      </div>

      {message && <p className="font-mono text-sm text-brass">{message}</p>}
        ...
        {items.map((item) => (
          <div key={item.id} className="dossier-card flex flex-col gap-2 p-4">
            ...
            <div className="mt-auto flex items-center justify-between border-t border-paper-dark pt-2 font-mono text-sm">
              <span className="font-semibold text-olive">{item.price.toLocaleString("fr-FR")} Cr.</span>
              <span className="text-paper-text/60">{item.stock === null ? "Illimité" : `${item.stock} en stock`}</span>
            </div>
            <button
              onClick={() => handleBuy(item.id)}
              disabled={item.stock === 0}
              className="border border-brass py-1 font-mono text-xs uppercase text-brass hover:bg-brass hover:text-ink disabled:opacity-40"
            >
              Acheter
            </button>
          </div>
