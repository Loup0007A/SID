  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] gap-4">
      <aside className="w-64 shrink-0 space-y-2 overflow-y-auto">
        <button onClick={() => setShowNew((s) => !s)} className="w-full bg-brass px-3 py-2 font-display text-sm uppercase text-ink hover:bg-brass-light">
        ...
        {showNew && (
          <div className="dossier-card space-y-3 p-3">
            ...
            <button onClick={createGroup} className="w-full bg-olive px-3 py-1 font-mono text-xs uppercase text-paper hover:bg-olive-light">
              Créer le groupe
            </button>
          </div>
        )}
        ...
        {channels.map((c) => (
          <button
            ...
            className={`block w-full rounded px-3 py-2 text-left font-mono text-sm ${
              activeChannel === c.id ? "bg-ink-border text-brass" : "hover:bg-ink-border"
            }`}
          >
        ...
        {!activeChannel ? (
          ...
          <>
            {(() => {
              ...
              return (
                <div className="mb-2 border border-redact/40 bg-redact/10 px-3 py-1 font-mono text-xs text-redact">
            ...
            <div className="flex-1 space-y-2 overflow-y-auto pr-2">
              {messages.map((m) => (
                <div key={m.id} className={`max-w-md rounded px-3 py-2 ${m.sender_id === userId ? "ml-auto bg-olive text-paper" : "bg-paper-dark"}`}>
                  {m.sender_id !== userId && (
            ...
            <form onSubmit={sendMessage} className="mt-3 flex gap-2 border-t border-paper-dark pt-3">
              <input
                ...
                className="flex-1 border border-paper-dark bg-paper px-3 py-2 font-body text-ink outline-none focus:border-olive"
              />
              <button type="submit" className="bg-brass px-4 font-display text-sm uppercase text-ink hover:bg-brass-light">
                Envoyer
              </button>
            </form>
          </>
