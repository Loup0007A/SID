...
import type { Role, Group, Profile, PermissionKey } from "@/types/database";
...
export default function RolesAdminPage() {
  ...
  return (
    <div className="space-y-10">
      <h1 className="font-display text-2xl uppercase tracking-wide text-brass">Rôles &amp; équipes</h1>
      ...
      {canRoles && (
        <section className="space-y-4">
          ...
          <form onSubmit={createRole} className="flex gap-2">
            ...
            <button className="bg-olive px-4 font-display text-sm uppercase text-paper hover:bg-olive-light">Créer</button>
          </form>

          <div className="space-y-4">
            {roles.map((role) => (
              <div key={role.id} className="dossier-card p-4">
          ...
          <form onSubmit={assignRoleToUser} className="dossier-card flex flex-wrap items-end gap-3 p-4">
            ...
            <button className="bg-brass px-4 py-2 font-display text-sm uppercase text-ink hover:bg-brass-light">Attribuer</button>
          </form>
        </section>
      )}
      ...
      {canTeams && (
        <section className="space-y-4">
          ...
          <form onSubmit={createGroup} className="dossier-card flex flex-wrap items-end gap-3 p-4">
            <div className="space-y-1">
            ...
            <label className="flex items-center gap-2 font-mono text-xs">
              <input type="checkbox" checked={newGroupIndependent} onChange={(e) => setNewGroupIndependent(e.target.checked)} />
              Système indépendant
            </label>
            <button className="bg-olive px-4 py-2 font-display text-sm uppercase text-paper hover:bg-olive-light">Créer</button>
          </form>

          <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {groups.map((g) => (
              <li key={g.id} className="dossier-card p-4">
                <p className="font-display uppercase">{g.name}</p>
                <p className="font-mono text-xs text-paper-text/60">{g.type}{g.is_independent_system ? " · système indépendant" : ""}</p>
              </li>
            ))}
          </ul>
        </section>
