...
export default function EspionageAdminPage() {
  ...
  return (
    <div className="space-y-10">
      <div>
        <p className="font-mono text-xs uppercase tracking-[0.3em] text-redact">Accès restreint</p>
        <h1 className="font-display text-2xl uppercase tracking-wide text-brass">Gestion du réseau d&apos;espionnage</h1>
      </div>
      ...
      <section className="space-y-3">
        ...
        <div className="space-y-2">
          {members.map((m) => {
            ...
            return (
              <div key={m.id} className="dossier-card flex flex-wrap items-center justify-between gap-2 p-3">
                <span className="font-body">{m.nickname}</span>
                {spy ? (
                  <span className="font-mono text-xs text-redact">
                    Espion · niveau {spy.spy_level} · {spy.xp} XP
                  </span>
                ) : (
                  <span className="font-mono text-xs text-paper-text/50">—</span>
                )}
                <button
                  onClick={() => toggleSpy(m.id, !spy)}
                  className={`border px-3 py-1 font-mono text-xs uppercase ${
                    spy ? "border-redact text-redact hover:bg-redact hover:text-paper" : "border-olive text-olive hover:bg-olive hover:text-paper"
                  }`}
                >
      ...
      <section className="space-y-3">
        ...
        {reports.map((r) => (
          <div key={r.id} className="dossier-card space-y-1 p-4">
            <p className="font-mono text-xs text-paper-text/60">
            ...
            {r.snippets.map((s, i) => (
              <p key={i} className="font-body text-sm">
                <span className="font-mono text-xs uppercase text-brass">{s.sender_nickname} : </span>
                {s.content}
              </p>
            ))}
          </div>
