...
export default function ApplicationsPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="font-display text-2xl uppercase tracking-wide text-brass">Candidatures en attente</h1>
          ...
          return (
            <div key={a.id} className="dossier-card grid grid-cols-1 gap-3 p-6 sm:grid-cols-2">
              ...
              <div className="flex flex-wrap gap-2 sm:col-span-2">
                <button onClick={() => saveDetails(a.id)} className="border border-teal px-4 py-2 font-mono text-xs uppercase text-teal hover:bg-teal hover:text-paper">
                  Enregistrer les modifications
                </button>
                <button onClick={() => openChat(a.id)} className="border border-brass px-4 py-2 font-mono text-xs uppercase text-brass hover:bg-brass hover:text-ink">
                  Discuter avec le candidat
                </button>
                <button onClick={() => decide(a.id, true)} className="bg-olive px-4 py-2 font-mono text-xs uppercase text-paper hover:bg-olive-light">
                  Accepter
                </button>
                <button onClick={() => decide(a.id, false)} className="bg-redact px-4 py-2 font-mono text-xs uppercase text-paper hover:opacity-90">
                  Refuser
                </button>
              </div>
            </div>
